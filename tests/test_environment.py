...
async def test_wait_for_docker_ready_raises_on_timeout(temp_dir, monkeypatch):
    ...
    monkeypatch.setattr("harbor_islo.environment._DOCKER_READY_TIMEOUT_SEC", 4)
    monkeypatch.setattr("harbor_islo.environment._DOCKER_READY_POLL_INTERVAL", 2)
...
async def test_upload_file_delegates_to_sdk(temp_dir, monkeypatch):
    ...
    with patch(
        "harbor_islo.environment.async_upload_file", new_callable=AsyncMock
    ) as mock_upload:
...
async def test_upload_dir_delegates_to_sdk(temp_dir, monkeypatch):
    ...
    with patch(
        "harbor_islo.environment.async_upload_dir", new_callable=AsyncMock
    ) as mock_upload:
...
async def test_download_file_delegates_to_sdk(temp_dir, monkeypatch):
    ...
    with patch(
        "harbor_islo.environment.async_download_file", new_callable=AsyncMock
    ) as mock_dl:
...
async def test_download_dir_delegates_to_sdk(temp_dir, monkeypatch):
    ...
    with patch(
        "harbor_islo.environment.async_download_dir", new_callable=AsyncMock
    ) as mock_dl:
...
async def test_upload_file_two_hop_cleans_temp_on_failure(temp_dir, monkeypatch):
    ...
    with patch("harbor_islo.environment.async_upload_file", new_callable=AsyncMock):
...
async def test_inline_gateway_rules_create_ephemeral_profile(temp_dir, monkeypatch):
    ...
    from harbor_islo.environment import GatewayConfig, GatewayRuleConfig
...
async def test_gateway_config_no_rules_still_creates_profile(temp_dir, monkeypatch):
    ...
    from harbor_islo.environment import GatewayConfig
...
class TestComposeWaitForMainContainer:
    ...
    async def test_raises_on_timeout(self, temp_dir, monkeypatch):
        ...
        with (
            patch.object(env, "_compose_exec", new=fake_compose_exec),
            patch("harbor_islo.environment.asyncio.sleep", new=fast_sleep),
        ):
